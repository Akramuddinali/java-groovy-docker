node()   {
      def dockerImageName= 'akramuddinali/javadedockerapp_$JOB_NAME:$BUILD_NUMBER'
      stage('SCM Checkout'){
         git 'https://github.com/Akramuddinali/java-groovy-docker.git'          
      }
      stage('Build'){
     ...
     stage('Build Docker Image'){         
      ...
      stage('Publish Docker Image'){
         withCredentials([string(credentialsId: 'dockerpwd', variable: 'dockerPWD')]) {
              sh "docker login -u akramuddinali -p ${dockerPWD}"
         }
